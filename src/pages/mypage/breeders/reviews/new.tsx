...
const ReviewseditNew = () => {
  ...
  const handleImageChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      const imageUrls: string[] = Array.from(files).map((file) => URL.createObjectURL(file));
      setDog((prevReview) => ({
        ...prevReview,
        reviewImgFiles: [...prevReview.images, ...imageUrls],
      }));
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const dataToSubmit = {
        ...
        title: dog.title,
        content: dog.content,
        reviewImgFiles: dog.images,
      };
      // 이미지 파일들을 문자열 배열로 변환
      const imgFilesAsStringArray = dog.images.map((imageUrl) => imageUrl.toString());
      // 데이터를 API로 전송
      const response = await axios.post(API_PATHS.getAdopter, {
        ...dataToSubmit,
        reviewImgFiles: imgFilesAsStringArray,
      });
    } catch (e) {
          ...
          <S.ReviewBox>
            <S.ReviewContentsBox>
              <MypageForm>
                    ...
                    <S.EditContetsBox>
                      ...
                      <div className="line">
                        ...
                        <div className="box">
                          ...
                          <S.Input placeholder="루카스" name="name" value={dog.name} onChange={handleChange} required />
                        </div>
                      </div>
                    </S.EditContetsBox>
                    <S.ReviewWritingBox>
                ...
                <S.ImageUploadBox>
                  <div className="box">
                    <S.Label>이미지 업로드 ({`${dog.images.length + '/' + 4}`})</S.Label>
                  </div>
                  <form action="">
                    <S.ImagesSeleted>
                      ...
                      <label htmlFor="image">{defaultFileName}</label>
